...
class EstimacaoNaoLinear:
    ...
    def  setDados(self, data,dataType= None,separador=';',decimal='.',glx=[], gly=[]):
        ...
        #-----------------------------------------------------------------------------
                        ...
                        if '.xlsx' in data[i]: #Caso  onde a extensão é .xlsx
                            ...
                            if len(nomes_planilhas) > 1:#Caso o .xlsx tenha mais de uma planilha de dados
                                ...
                                for i2 in data_variable:
                                    lista_dataframe.append(data_variable[i2])  # cria uma lista de dataframes usando o dicionário de dataframes
                            else:
                        ...
                        for j in range(len(lista_arquivos)):
                            if not controle :
                                if data[i] in lista_arquivos[j]:#traz o arquivo que contenha o mesmo nome antes do ponto
                                    ...
                                    if name1.replace(data[i],'') == '.xlsx' :#depois de encontrar o arquivo na pasta,a rotina agora tem nome e extensão para fazer a importação
                                        nomes_planilhas = ExcelFile(name1).sheet_names # função que traz a lista com títulos das  planilhas
                                        if len(nomes_planilhas) > 1:#caso que o arquivo .xlsx tem mais de uma planilha,assim será gerado um dataframe por planilha
                                            data_variable = read_excel(name1, sheet_name=nomes_planilhas)#dicionário com os dataframes das planilhas contida no arquivo .xlsx
                                            for i2 in data_variable:
                                                lista_dataframe.append(data_variable[i2])  # adiciona todos os dataframes a lista de dataframes
                                                controle = True
                                        else:#caso que o arquivo xlsx possui apenas uma planilha
                                            lista_dataframe.append(read_excel(name1))
                                            controle = True
                                    elif name1.replace(data[i], '') == '.csv' or name1.replace(data[i], '') == '.CSV':#caso que o arquivo é um .csv
            ...
            dataframe_geral.drop([col for col in dataframe_geral.columns if "Unnamed" in col], axis=1,inplace=True)#removes columns with unnamed title
